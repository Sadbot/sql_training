...
CREATE SCHEMA SampleDB AUTHORIZATION postgres;
SET search_path TO SampleDB;
...
CREATE INDEX idx_emp_postalcode ON Employees(postalcode);
...
CREATE INDEX idx_sup_companyname ON Suppliers(companyname); 
CREATE INDEX idx_sup_postalcode  ON Suppliers(postalcode); 
...
CREATE INDEX idx_cat_categoryname ON Categories(categoryname);  
...
CREATE INDEX idx_prod_categoryid  ON Products(categoryid);
CREATE INDEX idx_prod_productname ON Products(productname);
CREATE INDEX idx_prod_supplierid  ON Products(supplierid);
...
CREATE INDEX idx_cust_companyname ON Customers(companyname);
CREATE INDEX idx_cust_postalcode  ON Customers(postalcode);
...
CREATE INDEX idx_ord_custid         ON Orders(custid);
CREATE INDEX idx_ord_empid          ON Orders(empid);
CREATE INDEX idx_ord_shipperid      ON Orders(shipperid);
CREATE INDEX idx_ord_orderdate      ON Orders(orderdate);
CREATE INDEX idx_ord_shippeddate    ON Orders(shippeddate);
CREATE INDEX idx_ord_shippostalcode ON Orders(shippostalcode);
...
CREATE INDEX idx_ordd_orderid   ON OrderDetails(orderid);
CREATE INDEX idx_ordd_productid ON OrderDetails(productid);
...
CREATE INDEX idx_sco_testid_score ON Scores(testid, score);
...
select setval(pg_get_serial_sequence('Employees', 'empid'), (select max(empid) from Employees) ); 
...
select setval(pg_get_serial_sequence('Customers', 'custid'), (select max(custid) from Customers) ); 
...
select setval(pg_get_serial_sequence('Shippers', 'shipperid'), (select max(shipperid) from Shippers) );
...
GROUP BY date_part('year',O.orderdate);
...
FROM Orders AS O
  JOIN OrderDetails AS OD
    ON OD.orderid = O.orderid
GROUP BY empid, date_trunc('month',O.orderdate);
